...
export async function claimNextJob(db) {
  ...
  // Quando non c'è nessun job PENDING, la funzione SQL restituisce NULL sul
  // tipo composito analysis_jobs — ma PostgREST lo serializza come un
  // oggetto con tutti i campi null, non come null puro. "!data" da solo non
  // lo intercetta: un job "fantasma" arrivava fino a runAnalysis, che
  // falliva con "Job senza root_offer_id o request_id" — innocuo quando
  // capitava di rado col pulsante manuale, sistematico ora che il worker
  // gira ogni 5 minuti e la coda è vuota la maggior parte delle volte.
