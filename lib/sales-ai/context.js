...
async function fetchAttachmentImages(db, attachments) {
  ...
  for (const att of candidates) {
    ...
    try {
      const { data, error } = await db.storage.from('allegati-offerte').download(att.storage_path);
      if (error || !data) {
...
export async function buildOfferOpportunityContext(db, suppliedRootOfferId) {
  ...
  const [byOffer, byCommessa, byTag] = await Promise.all([
  ...
  const merged = mergeRows(byOffer, byCommessa, byTag).sort(
  ...
  const { images: attachmentImages, skipped: skippedAttachmentImages } = await fetchAttachmentImages(db, offerAttachments);
  ...
  if (attachmentImages.length) {
    warnings.push(`${attachmentImages.length} immagine/i allegata/e all'offerta inviate come contenuto visivo: puoi leggerne il contenuto reale, non solo il nome file.`);
  }
