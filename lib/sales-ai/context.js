...
export async function buildOfferOpportunityContext(db, suppliedRootOfferId) {
  ...
  // mano sull'opportunità — Rev.1 §06: stesso trattamento già in uso per le
  // richieste pre-offerta, incluse le immagini lette come contenuto visivo.
  const salesDb = db.schema('sales_ai');
  const { data: manualEvents, error: manualEventsError } = await salesDb
    .from('request_events')
  ...
  const { data: manualAttachments, error: manualAttachmentsError } = await salesDb
    .from('request_attachments')
    .select('id,nome_file,tipo_file,storage_path,dimensione_kb,caricato_da,created_at')
    .eq('root_offer_id', rootOfferId);
  if (manualAttachmentsError) throw manualAttachmentsError;
