...
import { aggiungiEventoOfferta } from "./actions";
...
// Stesso modulo di app/(app)/richieste/EventoForm.tsx, scope offerta
// invece di richiesta — Rev.1 §06.
export function EventoForm({ rootOfferId, offerNumber }: { rootOfferId: string; offerNumber: string }) {
  ...
  function salva() {
    ...
    startTransition(async () => {
      const res = await aggiungiEventoOfferta({
        rootOfferId,
        offerNumber,
        tipo,
        descrizione,
    ...
    <div className="panel" style={{ padding: 16, display: "flex", flexDirection: "column", gap: 12 }}>
      ...
      <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
        ...
        <span style={{ fontSize: 12, color: "var(--muted)" }}>
          Al salvataggio Sales AI rilegge l&apos;opportunità con questa informazione in più.
        </span>
      </div>
    </div>
  );
