-- Il tipo di ritorno cambia rispetto alla versione precedente (nuove colonne
-- di dettaglio), quindi la funzione va ricreata da zero.
drop function if exists sales_ai.triage_ingest(integer);

-- Rev.1 §02: "scrematura automatica" dell'Inbox commerciale. Il filtro
-- esistente (posta_mittenti_classe) escludeva solo i mittenti già
-- classificati a mano. Qui si aggiungono due livelli deterministici che
-- intercettano marketing/notifiche PRIMA che l'email arrivi al modello,
-- così "Da smistare" resta solo ciò che richiede davvero una decisione:
--
--   1) pattern tecnico del mittente (no-reply, notifiche automatiche,
--      mailer-daemon...) — non è mai una persona che scrive;
--   2) contenuto tipico di newsletter/marketing (link di disiscrizione
--      esplicito nel corpo) — non ci sono header email grezzi salvati,
--      quindi il proxy più affidabile disponibile è il testo stesso.
--
-- Un mittente già classificato CLIENTE o FORNITORE in posta_mittenti_classe
-- (la whitelist/blacklist leggera già esistente, editabile a mano) ha
-- sempre la precedenza: se conosciamo già la relazione commerciale, quella
-- singola email va comunque al modello anche se somiglia a una newsletter.
--
-- Il match sui domini di posta_mittenti_classe ora copre anche i
-- sottodomini (es. una entry "pens.com" esclude pure "t.pens.com"), non
-- solo l'uguaglianza esatta di prima.
